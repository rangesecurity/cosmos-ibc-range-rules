...
@Injectable()
export class DowntimeSlashService implements OnBlock {
  private readonly eventTypeToFilter = 'consumer_slash_request';
  private readonly eventAttributeKeyForInfractionTypeDowntime =
    'infraction_type';
  private readonly eventAttributeValueForInfractionTypeDowntimeValue =
  ...
  callback: (
    ...
    if (block.block_events) {
      ...
      for (const e of events.begin_block) {
        if (e.type === this.eventTypeToFilter) {
          ...
          if (
            attVal === this.eventAttributeValueForInfractionTypeDowntimeValue
          ) {
            targetEvents.push(e);
          }
        }
    ...
    const subEvents: ISubEvent[] = targetEvents.map((e) => {
      ...
      return {
        details: {
          message: `A consumer initiated downtime slashing request has been initiated for validator ${validator}.`,
        },
        txHash: '',
        addressesInvolved: [],
        severity: 'medium',
